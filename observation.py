...
def get_veh_dict():
  ...
  for veh_id in veh_id_list:
    ...
    # edge id before entering intersection (edge id on route)
    veh_dict[veh_id]["edge_id_route"] = route[traci.vehicle.getRouteIndex(veh_id)]
...
def get_obs():
  ...
  ego_next_edge_id = ego_dict["next_edge_id"]
  lane_id_list_ego_next_edge = edge_dict[ego_next_edge_id]["lane_id_list"]
  lane_id_list_ego_edge = edge_dict[ego_dict["edge_id_route"]]["lane_id_list"]
  for veh_id, state_dict in veh_dict_ROI.items():
    ...
    # it's inside the intersection, or
    # it's inside a lane of higher priority than ego
    if ((obs_dict["ego_in_intersection"] == 0) and 
        (edge_dict[ego_dict["edge_id_route"]]["to_node_id"] == edge_dict[state_dict["edge_id_route"]]["to_node_id"])
        ):
      if ((state_dict["edge_id"][0] == ':') or
          (edge_dict[state_dict["edge_id_route"]]["priority"] > edge_dict[ego_dict["edge_id_route"]]["priority"])
          ):
    ...
    lane_id_list_veh_edge = edge_dict[state_dict["edge_id_route"]]["lane_id_list"]
    ...
    if edge_dict[ego_dict["edge_id_route"]]["to_node_id"] == edge_dict[state_dict["edge_id_route"]]["to_node_id"]:
      ...
      # NEXT, LEFT, RIGHT
    if ((ego_dict["edge_id_route"] == state_dict["edge_id_route"] and \
         ego_next_edge_id == veh_next_edge_id and \
         state_dict["edge_id"][0] == ':') or
        (state_dict["edge_id"][0] != ':' and state_dict["edge_id"][0] == ':' and \
         state_dict["edge_id"] == ego_next_edge_id and
         len(set(lanelet_dict[state_dict["lane_id"]]["prev_lane_id_list"]) & edge_dict[ego_dict["edge_id_route"]]["lane_id_list"]) > 0)
         ):
