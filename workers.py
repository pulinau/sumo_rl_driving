...
def select_action(dqn_cfg_list, action_set_list, explr_set_list):
  valid = set()
  invalid = [] # invalid stores the exploration actions

  for action_set, explr_set, dqn_cfg in zip(action_set_list, explr_set_list, dqn_cfg_list):
    if len(valid) == 0:
      valid = valid | action_set
      invalid += [(x, "explr: " + dqn_cfg.name) for x in explr_set]
    elif len(valid & action_set) == 0:
      invalid += [(x, "explr: " + dqn_cfg.name) for x in valid]
      print("no available action for " + dqn_cfg.name)
      break
    else:
      invalid += [(x, "explr: " + dqn_cfg.name) for x in (explr_set & valid)]
    valid = valid & action_set
